...
function ArterialRoadService._calculateArterialPaths(zone_grid, params)
    ...
    -- DYNAMICALLY CALCULATE ARTERIALS if not provided
    local num_arterials = params.num_arterials or math.max(2, math.floor((width + height) / 75))
    ...
    for i = 1, num_arterials do
        ...
        local start_quad = start_quad_candidates[love.math.random(1, #start_quad_candidates)]
        ...
        if constraint_node then
            ...
            if entry_point then
                ...
                if path1 then
                    ...
                    -- FIX: Pass width and height to the exit point finder
                    exit_point = ArterialRoadService._findBestExitPoint(constraint_node, incoming_direction, all_boundary_points, entry_point, exit_edges, width, height)
                    if exit_point then
...
function ArterialRoadService._findBestExitPoint(constraint_node, incoming_direction, all_boundary_points, entry_point, allowed_exit_edges, width, height)
    ...
    -- FIX: Pass the correct width and height to the fallback function to prevent crash
    return ArterialRoadService._getRandomEdgePoint(width, height, "exit", entry_point, 0, nil, allowed_exit_edges, all_boundary_points)
end
...
function ArterialRoadService._getIncomingDirection(path)
    ...
    local dx, dy = end_node.x - start_node.x, end_node.y - start_node.y
...
function ArterialRoadService._createSpecializedCostGrid(base_grid, zone_grid, target_zone_info)
    local new_grid = ArterialRoadService._cloneCostGrid(base_grid)
    local low_cost = target_zone_info.cost
    if target_zone_info.tiles then 
        for _,tile in ipairs(target_zone_info.tiles) do 
            new_grid[tile.y][tile.x] = low_cost 
        end
    elseif target_zone_info.zone_name then 
        for y,row in ipairs(zone_grid) do 
            for x,zone in ipairs(row) do 
                if zone == target_zone_info.zone_name then 
                    new_grid[y][x] = low_cost 
                end 
            end 
        end 
    end
...
function ArterialRoadService._cloneCostGrid(original_grid)
    local new_grid={}
    for y,row in ipairs(original_grid) do 
        new_grid[y]={}
        for x,cost in ipairs(row) do 
            new_grid[y][x] = cost 
        end 
    end
    return new_grid
...
function ArterialRoadService._findLargestDistrict(zone_grid,width,height)
    local visited = {}
    for y = 1, height do visited[y] = {} end
    
    local largest_district = {tiles={}, center=nil, quadrant=nil}
    ...
    for y=1,height do 
        for x=1,width do 
            if not visited[y][x] and valid_zone_map[zone_grid[y][x]] then
                local current_district_tiles = {}
                local q = {{x=x,y=y}}
                visited[y][x] = true
                local zone_type = zone_grid[y][x]
                local head = 1
                while head <= #q do 
                    local curr=q[head]
                    head=head+1
                    table.insert(current_district_tiles,curr)
                    local neighbors={{curr.x,curr.y-1},{curr.x,curr.y+1},{curr.x-1,curr.y},{curr.x+1,curr.y}}
                    for _,pos in ipairs(neighbors) do 
                        local nx,ny = pos[1],pos[2] 
                        if nx>0 and nx<=width and ny>0 and ny<=height and not visited[ny][nx] and zone_grid[ny][nx]==zone_type then 
                            visited[ny][nx]=true
                            table.insert(q,{x=nx,y=ny})
                        end 
                    end
                end
                if #current_district_tiles > #largest_district.tiles then 
                    largest_district.tiles = current_district_tiles 
                end
            end 
        end 
    end
    ...
    if #largest_district.tiles > 0 then
        ...
        for _,tile in ipairs(largest_district.tiles) do 
            sum_x = sum_x+tile.x
            sum_y = sum_y+tile.y 
        end
        local center_x = math.floor(sum_x/#largest_district.tiles)
        local center_y = math.floor(sum_y/#largest_district.tiles)
        largest_district.center = {x=center_x,y=center_y}
        local mid_w,mid_h = width/2,height/2
        if center_y <= mid_h then 
            if center_x <= mid_w then largest_district.quadrant="top-left" else largest_district.quadrant="top-right" end 
        else 
            if center_x <= mid_w then largest_district.quadrant="bottom-left" else largest_district.quadrant="bottom-right" end 
        end
...
function ArterialRoadService._updateCostGridWithNewRoad(cost_grid,path)
    if not cost_grid or not path then return end
    local merge_cost=1
    for _,node in ipairs(path) do 
        if cost_grid[node.y] and cost_grid[node.y][node.x] then 
            cost_grid[node.y][node.x]=merge_cost 
        end 
    end
end
...
function ArterialRoadService._createCostGrid(zone_grid,width,height)
    local cost_grid = {}
    local boundary_cost = 5
    local district_interior_cost = 150
    for y = 1,height do 
        cost_grid[y] = {}
        for x = 1,width do 
            local current_zone = zone_grid[y][x]
            local is_boundary = false
            local neighbors={{x,y-1},{x,y+1},{x-1,y},{x+1,y}}
            for _,pos in ipairs(neighbors) do 
                local nx,ny = pos[1],pos[2]
                if nx > 0 and nx <= width and ny > 0 and ny <= height and zone_grid[ny][nx] ~= current_zone then 
                    is_boundary=true
                    break 
                end 
            end
...
function ArterialRoadService._getRandomEdgePoint(width,height,point_type,other_point,min_distance,zone_grid,allowed_edges,all_boundary_points)
    ...
    for _,point in ipairs(boundary_points) do 
        local is_valid = true
        if allowed_edges then 
            local is_allowed=false
            for _,allowed in ipairs(allowed_edges) do 
                if point.edge==allowed then is_allowed=true; break end 
            end
            if not is_allowed then is_valid=false end 
        end
        if is_valid and other_point and min_distance then 
            if math.sqrt((point.x-other_point.x)^2+(point.y-other_point.y)^2)<min_distance then is_valid=false end
            if point.edge==other_point.edge then is_valid=false end 
        end
...
function ArterialRoadService._getRandomEdgePointFallback(width,height,point_type,other_point,min_distance,allowed_edges)
    local edges={{name="top",points={}},{name="bottom",points={}},{name="left",points={}},{name="right",points={}}}
    for x=1,width do 
    ...
    for _,edge in ipairs(edges) do 
        local edge_is_allowed = not allowed_edges
        if allowed_edges then 
            for _,allowed in ipairs(allowed_edges) do 
                if edge.name==allowed then edge_is_allowed=true; break end 
            end 
        end
        if edge_is_allowed then 
...
function ArterialRoadService._findDistrictBoundaryPoints(zone_grid,width,height)
    ...
    for x=2,width do 
        if zone_grid[1][x]~=zone_grid[1][x-1] then table.insert(boundary_points,{x=x,y=1,edge="top",zone_from=zone_grid[1][x-1],zone_to=zone_grid[1][x]}) end
        if zone_grid[height][x]~=zone_grid[height][x-1] then table.insert(boundary_points,{x=x,y=height,edge="bottom",zone_from=zone_grid[height][x-1],zone_to=zone_grid[height][x]}) end 
    end
    for y=2,height do 
        if zone_grid[y][1]~=zone_grid[y-1][1] then table.insert(boundary_points,{x=1,y=y,edge="left",zone_from=zone_grid[y-1][1],zone_to=zone_grid[y][1]}) end
        if zone_grid[y][width]~=zone_grid[y-1][width] then table.insert(boundary_points,{x=width,y=y,edge="right",zone_from=zone_grid[y-1][width],zone_to=zone_grid[y][width]}) end 
    end
...
function ArterialRoadService._smoothPath(path)
    ...
    for i=2,#path-1 do 
        ...
        if(p2.x-p1.x)~=(p3.x-p2.x)or(p2.y-p1.y)~=(p3.y-p2.y)then 
            table.insert(smoothed,p2)
        end 
    end
